...
select * from customers inner join orders on orders.customer_id = customers.customer_id where orders.`status` = "Cancelled";

# Q2) Fetch the details of customers who have done payments between the amount 5,000 and 35,000?
select customers.*, payments.amount from customers inner join payments on payments.customer_id =  customers.customer_id where payments.amount between 5000 and 35000 order by payments.amount;
...
Job Title: Sales Rep
*/
insert into employees values(15657,"Roy","Lakshmi","x4065","lakshmiroy1@lcomotors.com",4,1088,"Sales Rep");
...
select distinct products.*,orders.`status` from products inner join orderdetails on orderdetails.product_code = products.product_code inner join orders on orders.order_id = orderdetails.order_id where  orders.`status` = "Shipped" and products.product_line = "Motorcycles";

# Q6) Write a SQL query to get details of all employees/salesmen in the office located in Sydney?
select * from employees where office_code = (select office_code from offices where city = "sydney");

# Q7) How would you fetch the details of customers whose orders are in process?
select * from customers where customer_id = (select customer_id from orders where `status` = "In_process");

# Q8) How would you fetch the details of products with less than 30 orders?
select distinct products.*, count(orderdetails.order_id) as number_of_orders from products inner join orderdetails on orderdetails.product_code = products.product_code group by orderdetails.product_code having count(orderdetails.order_id) < 30;
...
select distinct employees.*,customers.customer_name,orders.`status` from employees inner join customers on customers.sales_employee_id = employees.employee_id inner join orders on orders.customer_id = customers.customer_id where orders.status = "resolved";

# Q11) Get the details of the customer who made the maximum payment?
select customers.*,payments.amount from customers inner join payments on payments.customer_id = customers.customer_id order by payments.amount desc limit 1;

# Q12) Fetch list of orders shipped to France?
select orderdetails.*, orders.*,customers.country from orderdetails inner join orders on orders.order_id = orderdetails.order_id inner join customers on customers.customer_id = orders.customer_id where customers.country = "France";

# Q13) How many customers are from Finland who placed orders?
select distinct customers.* from customers inner join orders on orders.customer_id = customers.customer_id where customers.country = "finland";
...
select * from customers where customer_id in (select customer_id from payments where payment_date between '2019-05-01' and '2019-06-30');

# Q16) How many orders shipped to Belgium in 2018?
select count(*) as number_orders from orders where customer_id in (select  customer_id from customers where country = "belgium") and `status` = "shipped" and shipped_date between '2018-01-01' and '2018-12-30';

# Q17) Get the details of the salesman/employee with offices dealing with customers in Germany?
select distinct employees.*,offices.*,customers.country from employees inner join offices on offices.office_code = employees.office_code inner join customers on customers.sales_employee_id = employees.employee_id where customers.country = "Germany";
...
Status: In Process
*/
insert into orderdetails values(10426,"S12_3148",41,151,11);
insert into orders values(10426,'2022-10-10','2022-10-20',null,'In process',null,496);

# Q19) Fetch details of employees who were reported for the payments made by the customers between June 2018 and July 2018?
select distinct employees.* from employees inner join customers on customers.sales_employee_id = employees.employee_id inner join payments on payments.customer_id = customers.customer_id where payments.payment_date between '2018-06-01' and '2018-07-31';
...
Amount : 33789.55
*/
insert into payments values (119,"OM314944","2022-10-10","33789.55");

# Q21) Get the address of the office of the employees that reports to the employee whose id is 1102?
select * from offices where office_code = (select office_code from employees where employee_id = 1102);

# Q22) Get the details of the payments of classic cars?
select distinct payments.*,products.product_line from payments inner join customers on customers.customer_id = payments.customer_id inner join orders on orders.customer_id = customers.customer_id 
inner join orderdetails on orderdetails.order_id = orders.order_id inner join products on products.product_code = orderdetails.product_code where products.product_line = "Classic Cars";

# Q23) How many customers ordered from the USA?
select distinct count(orders.customer_id) as number_of_customers ,offices.country from orders inner join customers on customers.customer_id = orders.customer_id inner join employees on employees.employee_id = customers.sales_employee_id inner join offices on offices.office_code = employees.office_code where offices.country = "USA";
...
select employees.employee_id, employees.first_name ,employees.last_name, employees.email, employees.job_title, employees.extension,  offices.office_code, offices.address_line1, offices.address_line2, offices.phone, offices.city, offices.state, offices.country, offices.postal_code, offices.territory from employees left join offices on offices.office_code = employees.office_code where offices.country = "USA";

# Q26) Fetch total price of each order of motorcycles. (Hint: quantity x price for each record)?
select orderdetails.order_id, orderdetails.each_price, products.product_line,orderdetails.quantity_ordered, (orderdetails.quantity_ordered * orderdetails.each_price) as sum_of_motorcycle from products inner join orderdetails on orderdetails.product_code = products.product_code where products.product_line = "motorcycles";

# Q27) Get the total worth of all planes ordered?
select sum(orderdetails.quantity_ordered*orderdetails.each_price) as sum_of_planes from products inner join orderdetails on orderdetails.product_code = products.product_code where product_line = "planes";

# Q28) How many customers belong to France?
select count(*) number_of_customers from customers where country = "france";

# Q29) Get the payments of customers living in France?
select * from payments where customer_id in (select customer_id from customers where country = "france");

# Q30) Get the office address of the employees/salesmen who report to employee?
select * from offices where office_code in (select office_code from employees where reports_to = 1143);
